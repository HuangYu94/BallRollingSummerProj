function OneBallLolli(ang_x,ang_y)
...
while Zerror>precision
    stp=stp+1;
    step1(stp)=stp;
    StepArc=FindArc(configBall,R,rc,rad);
    tra_arc(stp)=StepArc;
    %make the lollipop rotation:
    configX=ArbAxisRotate([0,-R,0],[0,0,rad],-StepArc*R/rc,configBall);
    configBall=ArbAxisRotate([0,-R,0],[0,-R,1],StepArc,configX);
...
function stepArc=FindArc(configBall,R,rc,rad)
...
for i=1:trial_num
    ...
    configXtemp=ArbAxisRotate([0,-R,0],[0,0,rad],-try_value*R/rc,configBall);
    configXtemp=ArbAxisRotate([0,-R,0],[0,-R,1],try_value,configXtemp);
    %calculate the zaxis of the ball after rotation
    zaxistemp=configXtemp(:,2)-configXtemp(:,1);
